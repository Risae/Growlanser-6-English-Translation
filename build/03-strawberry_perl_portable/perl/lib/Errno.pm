...
"MSWin32-x64-multi-thread-10.0.19041.388" or
	die "Errno architecture (MSWin32-x64-multi-thread-10.0.19041.388) does not match executable architecture ($Config{'archname'}-$Config{'osvers'})";
...
our %EXPORT_TAGS = (
    ...
    WINSOCK => [qw(
	WSAEINTR WSAENOBUFS WSAEMFILE WSAENOPROTOOPT WSAEINVALIDPROVIDER
	WSAEUSERS WSAEISCONN WSAEHOSTDOWN WSAECONNRESET WSAEDISCON WSAENOMORE
	WSAEPROTONOSUPPORT WSAEACCES WSAESOCKTNOSUPPORT WSAEDQUOT
	WSAETOOMANYREFS WSAENOTEMPTY WSAEALREADY WSAEPROVIDERFAILEDINIT
	WSAEBADF WSAEREMOTE WSAETIMEDOUT WSAECONNREFUSED WSAEOPNOTSUPP
	WSAENETDOWN WSAENETUNREACH WSAEINVAL WSAEREFUSED WSAEPROCLIM
	WSAECANCELLED WSAEFAULT WSAEADDRNOTAVAIL WSAESHUTDOWN WSAESTALE
	WSAEINVALIDPROCTABLE WSAENAMETOOLONG WSAEWOULDBLOCK WSAENETRESET
	WSAECONNABORTED WSAENOTSOCK WSAEADDRINUSE WSAEMSGSIZE WSAEPROTOTYPE
	WSAEINPROGRESS WSAEAFNOSUPPORT WSAEHOSTUNREACH WSAENOTCONN
	WSAEDESTADDRREQ WSAELOOP WSAEPFNOSUPPORT
    )],
);
